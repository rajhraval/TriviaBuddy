...
protocol TBNavigationSearchDelegate: AnyObject {
    func sortTapped()
    func filterTapped()
    func didSearch(for textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool
}

protocol TBNavigationActionDelegate: AnyObject {
    func leftButtonTapped()
    func rightButtonTapped()
}

final class TBNavigationView: UIView {

    weak var actionDelegate: TBNavigationActionDelegate?
    weak var searchDelegate: TBNavigationSearchDelegate?
    ...
    var leftBarButton: TBButton? = nil {
        didSet {
            setup()
        }
    }

    var rightBarButton: TBButton? = nil {
        didSet {
            setup()
        }
    }
    ...
    private var backButton: TBButton = {
    ...
    private var searchField: TBTextField = {
        let textField = TBTextField()
        textField.placeholder = "Search"
        return textField
    }()

    private var filterButton: TBButton = {
        let button = TBButton(style: .symbol)
        button.image = UIImage(systemName: "slider.horizontal.3")!
        button.backgroundColour = .systemIndigo
        return button
    }()

    private var sortButton: TBButton = {
        let button = TBButton(style: .symbol)
...
extension TBNavigationView {

    @objc 
    private func filterTapped(_ sender: TBButton) {
        searchDelegate?.filterTapped()
    }

    @objc 
    private func sortTapped(_ sender: TBButton) {
        searchDelegate?.sortTapped()
    }

    @objc
    private func leftBarButtonTapped(_ sender: TBButton) {
        actionDelegate?.leftButtonTapped()
    }

    @objc
    private func rightBarButtonTapped(_ sender: TBButton) {
...
extension TBNavigationView: UITextFieldDelegate {
